function getStatisticsData()
  {
    $.getJSON("core/controller/sample.php?job=get_tests_statistics", function(result){
      //console.log(result.data[0].pending);
      $('#pending_samples').html(result.data[0].pending);
      $('#succeeded_samples').html(result.data[0].passed);
      $('#tocheck_samples').html(result.data[0].tocheck);
      $('#failed_samples').html(result.data[0].failed);
      });
...
$(document).ready(function(){
  ...
  var table_view = $('#calls_data').dataTable({
    "ajax": "core/controller/sample.php?job=get_latest_tests",
    "columns": [
  ...
  setInterval(function() {table_view.api().ajax.reload(null, false); console.log("Updating");getStatisticsData();}, 5000);
  ...
  // Delete 
  $(document).on('click', '.function_repeat', function(e){
    e.preventDefault();
    var sample_number = $(this).data('name');
    if (confirm("Are you sure you want to repeat sample of '" + sample_number + "'?")){
      show_loading_message();
      var id      = $(this).data('id');
      var request = $.ajax({
        url:          'core/controller/sample.php?job=repeat_sample&id=' + id,
        cache:        false,
        dataType:     'json',
        contentType:  'application/json; charset=utf-8',
        type:         'get'
      ...
      request.done(function(output){
        if (output.result == 'success'){
          ...
          table_view.api().ajax.reload(function(){
            hide_loading_message();
            show_message("Sample of '" + sample_number + "' queued successfully.", 'success');
          }, true);
        } else {
          hide_loading_message();
